...
for (i,j) in blist.iterrows():
    ...
    while( (next_module!="FalsePositive")and  (next_module!="RiskEntity")):
        verdict=""
        final_list=[]
        interim_id_list_count = interim_id_list.__len__()
################################################################     
...
################################################################            
    ...
    print("We are out of WHile"+verdict)
    id_list=[]
    #final_data_frame=[]
    if(final_list.__len__()):
        id_list = [j for (i,j) in final_list]
    d['verdict'] = verdict
...
print(final_df)
pd.DataFrame(final_df).to_csv(open("_data\sa.csv",'w'))
