...
import pandas as pd
import json
config=json.loads(open("_data/test/AML_Flow.json").read())
blist = pd.DataFrame(json.load(open('_data/test/BankData.json','r')),index={1})
...
################################################################            
    ...
    if(final_list.__len__()):
        id_list = [i for (i,j) in final_list]#[j for (i,j) in final_list]
    d['verdict'] = verdict
